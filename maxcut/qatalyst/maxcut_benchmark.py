...
This benchmark script has been adapted from the maxcut_benchmark.py script for 
the ocean API.

To enable support for Qatalyst, install qci-client with:

pip install qci_client==1.4.5
...
sys.path[1:1] = [ "_common", "_common/qatalyst", "maxcut/_common" ]
sys.path[1:1] = [ "../../_common", "../../_common/qatalyst", "../../maxcut/_common/" ]
import common
...
def create_circ(nqubits, edges):
    ...
    circuitProxy.h = h
    circuitProxy.J = J
    circuitProxy.num_qubits = nqubits
...
def MaxCut (num_qubits, secret_int, edges, measured = True):
    ...
    if do_compute_expectation:
        ...
        compute_expectation(circuitProxy, num_qubits, secret_int)
...
def compute_expectation(qc, num_qubits, secret_int, backend_id='statevector_simulator', params=None):
    ...
    raise NotImplementedError("This method is not implemented for Qatalyst")
...
def compute_cutsizes(results, nodes, edges):
    ...
    cuts = [list(map(int, list(cut))) for cut in cuts]
    cuts = [[str(-1 * val + 1) for val in cut] for cut in cuts]
    cuts = ["".join(cut) for cut in cuts]
...
### DEVNOTE: this only applies for Qiskit, not Qatalyst
...
def load_all_metrics(folder, backend_id=None):
    ...
    for width_files in list_of_files:
        ...
        metrics.finalize_group_2_level(str(num_qubits))
...
def run (min_qubits=3, max_qubits=MAX_QUBITS, max_circuits=1, num_shots=1,
        ...
        backend_id='eqc1', provider_backend=None, eta=0.5,
    ...
    backend_id : string, optional
        DESCRIPTION. The default is 'eqc1'.
    ...
    print("MaxCut Benchmark Program - Qatalyst")
    ...
    ex.finalize_execution(metrics.finalize_group, report_end=False)
    ex.finalize_execution(metrics.finalize_group_2_level, report_end=True)
